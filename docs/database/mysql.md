...
# 实践
## 普通索引,唯一索引的选择
- 唯一索引无法使用change buffer,所以尽量考虑使用普通索引
## 给字符串字段加索引
- 考虑使用固定长度的索引
  - 优点: 节约索引的长度
  - 缺点: 可能会查出多条数据,会返表,不能使用覆盖索引的效果
- 考虑将部分特征值提出成一个字段,给该字段加索引
## 数据库查询卡顿
- 可能是在刷脏页入库,注意设置刷脏页的配置.
## 删除表数据无法该表表大小
- innodb_file_pre_table 该属性设置为ON 表示每张表单独文件保存,OFF表示表数据放在系统共享表空间内,建议ON
- delete 只是把记录的位置,或者数据页标记为可复用,并不会回收表空间
- 可以使用重建表来消除索引空洞,达到缩小表空间的目的,命令:alter table A engine=innoDB
